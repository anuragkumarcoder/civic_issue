...
const Alert = ({ 
  ...
  return (
    <div className={`border rounded-md p-4 mb-4 ${alertStyle}`} role="alert">
      <div className="flex items-start">
        {showIcon && (
          <div className={`flex-shrink-0 mr-3 ${iconStyle}`}>
            {icon}
          </div>
        )}
        ...
        {dismissible && onDismiss && (
          <button 
            type="button" 
            className="ml-auto -mx-1.5 -my-1.5 rounded-lg focus:ring-2 p-1.5 inline-flex h-8 w-8 hover:bg-opacity-10 hover:bg-gray-500"
            onClick={onDismiss}
            aria-label="Close"
          >
 ...
 * 
 * @param {Object} props - Props to pass to Alert component
 */
